...
- OpenAL is the default audio backend, providing spatial audio controls through `s_alReverb` and `s_alOcclusion`. Use `s_alDevice` to pick a specific OpenAL device, or switch back to the original mixer with `s_backend legacy`.
